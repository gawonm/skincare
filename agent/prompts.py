...
class PromptCatalog:
    ...
    _TEMPLATES: ClassVar[dict[PromptPurpose, str]] = {
        PromptPurpose.UNDERSTAND_REQUEST: (
            ...
            "효능·주의 질문에 성분이 명시되면 rag_route=evidence_only를 선택하세요. "
            "성분이 정해지지 않은 피부 고민에서 성분이나 제품을 찾는 요청은 "
            "rag_route=claim_then_evidence를 선택하세요. Claim은 후보 탐색일 뿐 검증 근거가 "
            "아니므로 두 경로를 바꾸거나 생략하지 마세요."
        ),
