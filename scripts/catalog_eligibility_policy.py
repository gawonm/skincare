...
class CatalogEligibilityPolicy:
    ...
    def evaluate(self, hit: OliveYoungGlobalCategoryHitFields) -> CatalogEligibilityResult:
        ...
        if hit.sell_stat_code != self._SELL_STAT_CODE_ON_SALE or hit.srch_psblt_yn != self._SEARCHABLE:
        ...
        if any(keyword in name_lower for keyword in self._BUNDLE_KEYWORDS) or _VOLUME_BUNDLE_PATTERN.search(
            hit.prdt_name
        ):
