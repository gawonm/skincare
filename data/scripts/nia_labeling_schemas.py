...
class NiaFrequencyPeriod(StrEnum):
    ...
    WEEK = "week"
    MONTH = "month"
    # "격일"처럼 매일도 아니고 특정 요일 지정도 아닌 주기를 min/max 횟수로 억지로
    # 표현하면(예: 주 3~4회) 원문의 "정확히 이틀에 한 번"이라는 규칙성이 사라진다.
...
class NiaIngredientSubject(BaseModel):
    ...
    def _check_id_requires_matched(self) -> "NiaIngredientSubject":
        if (
            ...
            and self.matching_status != NiaIngredientMatchingStatus.MATCHED
        ):
...
class NiaSourceMetadata(BaseModel):
    ...
    kind: str  # 레코드 출처 종류. 현재는 "training_zip_record" 하나뿐이지만, 추후 다른 출처
    # (예: 수동 검토 큐)가 생겼을 때 구분하기 위해 값 집합을 enum으로 고정하지 않고 보존한다.
    record_id: str
    source_archive: str
...
class NiaIngredientEffectClaimStatement(NiaStatementBase):
    ...
    # 원문이 "0.1~1.0% 농도에서"처럼 사용 농도를 명시한 경우에만 채운다. object에 섞어
    # 넣으면 정성적 효능 서술과 정량 정보가 구분되지 않아 나중에 파싱할 수 없다.
...
class NiaCombinationClaimStatement(NiaStatementBase):
    ...
    def _check_explicit_combination(self) -> "NiaCombinationClaimStatement":
        ...
        if self.subject_plural_mode == NiaSubjectPluralMode.UNCERTAIN and not self.note:
            raise ValueError(
                "subject_plural_mode=uncertain인 combination_claim은 note로 근거를 남겨야 함"
            )
        return self
...
class NiaLabelingSpanVerifier:
    ...
    def _resolve_path(self, raw_record: dict, json_path: str):
        assert json_path.startswith(self._SIMPLE_PATH_PREFIX), (
            f"지원하지 않는 json_path 형식: {json_path}"
        )
        current = raw_record
