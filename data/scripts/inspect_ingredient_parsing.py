...
class IngredientParsingInspector:
    ...
    def run(self) -> None:
        ...
        summary_lines = [
            ...
            "구간 연결 상태 분포: "
            + ", ".join(f"{k}={v}" for k, v in sorted(link_status_counts.items())),
        ]
        summary = "\n".join(summary_lines)
    ...
    def _read_rows_with_ingredients_text(self, csv_path: Path) -> list[dict[str, str]]:
        ...
        with csv_path.open("r", encoding="utf-8-sig", newline="") as file:
            ...
            return [
                row
                for row in reader
                if row["source"] == DataSource.OLIVEYOUNG_GLOBAL.value
                and row["raw_ingredients_text"]
            ]
    ...
    def _format_product(
        ...
        for section in result.sections:
            ...
            lines.append(
                f"  [{label}] link={section.link_status.value} gds_cd={section.linked_option_gds_cd}"
            )
            for token in section.tokens:
                if token.parse_status == IngredientTokenParseStatus.NEEDS_REVIEW:
