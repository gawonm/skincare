...
`data/scripts/inspect_ingredient_parsing.py`(DB에 안 쓰고 리포트만 만드는 버전)와 같은 읽기·
파싱 경로를 쓰되, 여기서는 실제로 DB에 커밋한다.

사용법:
    uv run python -m data.scripts.ingest_product_ingredients
"""
...
from data.scripts.product_ingredient_text_parser import PARSER_VERSION, ProductIngredientTextParser
...
async def _run() -> None:
    ...
    ) + _read_rows_with_ingredients_text(_CATALOG_CSV_PATH)
