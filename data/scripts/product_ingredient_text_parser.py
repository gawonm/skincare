...
# 필요한 정보라 제거하면 안 된다.
_CONCENTRATION_INNER_PATTERN = re.compile(r"^\d[\d,.\s]*\s*(%|ppm|ppb|mg|µg|㎍)$", re.IGNORECASE)
...
class ProductIngredientTextParser:
    ...
    def _find_suspected_missing_separator_pairs(self, tokens: list[str]) -> set[int]:
        ...
        for index in range(len(tokens) - 1):
            if _TRAILING_DIGIT_SUFFIX.match(
                tokens[index]
            ) and _KNOWN_MERGED_DIOL_CONTINUATION.match(tokens[index + 1]):
                suspected.add(index)
